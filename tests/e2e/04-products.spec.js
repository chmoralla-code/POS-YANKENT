...
test.describe('Products & Inventory', () => {
  ...
  test('Newly Added Items chip shows and filters all 20 requested products', async () => {
    const { electron, page } = await launchApp();
    try {
      await login(page, 'admin', 'admin123');
      await navigate(page, 'products');
      ...
      await expect(chip).toContainText(/Newly Added Items\s*20/);
      await chip.click();
      await expect(page.locator('#pGrid .prod-card')).toHaveCount(20);
